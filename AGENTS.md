...
## 规则十三：文档按生命周期归位（Docs by lifecycle）
一事一家：决策进 docs/design.md，当前状态进 docs/overview.md，实验证据进 experiments/，运行说明进 README.md。同一事实出现在两处即违规。
overview.md 永远是当前快照、零历史（出现"以前/曾经"即移走）；experiments/ 每个实验须配一行 leaderboard 和一句决策（保留或回滚 + 原因），无决策句不算完成。
doc 存理由不存结构：接口、字段、签名以代码为准，doc 只链接不复制；README 指向 design.md，不复述其内容。
不为不存在的问题或受众建 doc（见规则二）：新增任何 doc 文件须先论证"现有四个家装不下"，否则禁建。